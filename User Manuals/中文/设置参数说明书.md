...
play_midi_start_process_time: 播放MIDI文件时开启发送MIDI事件的进程所需的时间，单位为秒（仅当 'midi_playing_multiprocess' 设置为 True 时才有效）

move_progress_adjust_time: 改变进度条的延迟时间，单位为秒（仅当 'midi_playing_multiprocess' 设置为 True 时才有效）

use_soundfont_delay_time: 使用SoundFont文件播放MIDI文件时在`bars drop`音符模式下的延迟时间，单位为秒（仅当 'midi_playing_multiprocess' 设置为 True 时才有效）

bars_mode_delay_time: 当音符模式为`bars`时的延迟时间（仅当 'midi_playing_multiprocess' 设置为 True 时才有效）
...
note_mode: 选择音符显示模式，目前提供以下3种模式：

1、音符条上升（对应值：'bars'）

2、音符条下落（只有播放MIDI文件模式可用）（对应值：'bars drop'）
...
default_audio_driver: fluidsynth使用的默认音频驱动，在一些系统上如果出现使用SoundFont文件播放MIDI文件时音频声音有问题的情况，可以通过修改这个参数设置为当前系统兼容的音频驱动
