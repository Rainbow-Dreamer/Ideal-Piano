...
2. 使用pip来安装以下的python库。`pip install pygame==2.1.2 pyglet==1.5.11 mido_fix pydub py pyqt5 pyinstaller`。
...
6. 在`Ideal Piano/`路径下打开终端，运行`pyinstaller -w -F "Ideal Piano start program.pyw"`，等待编译。如果你想添加图标，那么在它后面添加`--icon="resources/piano.ico"`。(在macOS上，图标文件名是`piano_icon.icns`)
