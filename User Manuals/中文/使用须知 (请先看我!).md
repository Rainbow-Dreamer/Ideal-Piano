...
点击`PLAY`按钮进入电脑键盘演奏模式，点击`MIDI KEYBOARD`按钮进入MIDI键盘演奏模式，右键点击`MIDI KEYBOARD`按钮打开选择MIDI设备的界面点击`PLAY MIDI`按钮进入播放MIDI文件模式。在进入其中一个模式之后，点击`GO BACK`按钮返回到初始页面。点击`SETTINGS`按钮打开更改设置的界面。
