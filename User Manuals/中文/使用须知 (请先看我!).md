...
对于Linux版本，使用默认设置播放MIDI文件需要确保当前有可以发出声音的MIDI输出端口，然后在选择MIDI设备的界面中选择对应的MIDI输出端口。

Linux版本（尤其是Ubuntu）最推荐的MIDI文件播放方案是使用FluidSynth + QSynth，用户可以自由选择本地的SoundFont文件作为音源播放MIDI文件。
...
然后打开QSynth的界面，设置适配于当前系统版本的MIDI输入和输出的音频启动，并选择SoundFont音源文件（FluidSynth提供默认的SoundFont音源文件可供选择）。

然后在Ideal Piano的选择MIDI设备的窗口中选择MIDI输出端口为QSynth设置的MIDI端口即可。



注意：如果在这里找不到可供选择的MIDI输出端口，打开终端执行以下指令，然后重新打开Ideal Piano试一下。
...
如果你想在Linux版本中使用SoundFont文件作为乐器，你需要安装FluidSynth，你可以参考[这里](https://github.com/FluidSynth/fluidsynth/wiki/Download)了解不同Linux发行版的安装命令。



另一种方案是安装freepats和timidity。在Ubuntu上，你可以运行

```
sudo apt-get install freepats timidity
```

然后打开终端，运行以下命令以打开 timidity 的 MIDI 端口：
...
然后在Ideal Piano的选择MIDI设备的窗口中选择MIDI输出端口为timidity的MIDI端口即可。
...
如果你目前没有任何MIDI端口与合成器，这里建议安装VMPK (Virtual MIDI Piano Keyboard)，点击[这里](https://sourceforge.net/projects/vmpk/files/vmpk/0.8.8/vmpk-0.8.8-mac-x64.dmg/download)下载安装包，安装好之后打开VMPK，从菜单栏打开`Edit - MIDI Connections `，勾选`Enable MIDI Input` 和 `Enable MIDI Thru on MIDI Output`, 然后在`MIDI IN Driver`那一栏选择`CoreMIDI`，然后点击`OK`保存设置。接下来在Ideal Piano中右键点击`MIDI KEYBOARD`按钮打开选择MIDI端口的窗口，在`MIDI Output Driver`那一栏选择`CoreMIDI,MIDI In`,关闭窗口。然后就可以正常播放MIDI文件了。

Linux版本也可以使用[VMPK](https://sourceforge.net/projects/vmpk/files/vmpk/0.8.8/vmpk-0.8.8-x86_64.AppImage/download), 在Ideal Piano的选择MIDI设备的窗口中，`MIDI IN Driver`那一栏选择`ALSA`，`MIDI Output Driver`那一栏选择`ALSA, in`即可。

如果你想在macOS版本中使用SoundFont文件作为乐器，你需要在macOS上安装FluidSynth，推荐使用MacPorts来安装FluidSynth。

请在当前的macOS系统中安装MacPorts，可以参考这个[链接](https://www.macports.org/install.php)安装。

然后在terminal运行这一行即可用MacPorts安装FluidSynth。
...
2. 确保你已经在你的环境中安装了 python3。

3. 使用pip来安装以下的python库：
...
7. 在`Ideal Piano/`路径下打开终端，运行以下指令：

   `pyinstaller -w -F "Ideal Piano start program.pyw" --hidden-import dataclasses --hidden-import=difflib --hidden-import=numpy`

   然后等待编译完成。如果你想添加图标，那么在它后面添加`--icon="resources/piano.ico"`。(在macOS上，图标文件名是`piano_icon.icns`)

8. 编译完成后，你可以在`dist`文件夹中找到可执行文件，并将其移动到`Ideal Piano/`路径中使用。



注意1：如果在Linux上pip3 install无法安装python库，以下为Ubuntu 24.04.4实测可行的方案：
...
注意2：如果在某些Linux系统上，使用SoundFont文件播放MIDI文件，声音听起来非常失真，请尝试通过更改设置参数文件中的'default_audio_driver'为当前Linux系统兼容的音频驱动，例如在Ubuntu 24.04.4，使用默认值'alsa'存在不兼容的情况，修改为'pulseaudio'可以修复播放时声音失真的问题。
