...
这个软件的各种参数设置都可以点击主屏幕上的`SETTINGS`按钮打开更改设置的界面修改，保存之后关闭更改设置的界面，改变会立即生效。
...
目前musicpy的分析曲子调性的算法正在测试改进中，你可以将`show_current_detect_key`设置为`True`，在你演奏或者播放MIDI文件时，屏幕上会显示当前的调性。你可以参考参数说明书里相关的参数说明来修改分析调性的算法的参数，以达到不同的效果。
...
1. 由于我这个软件是全英文的，考虑到很多小伙伴可能看中文的和弦类型名称比较亲切一些，因此我做了一个中文补丁包，请将设置文件里的`language`改为`'Chinese'`，保存后重启软件即可使用。
